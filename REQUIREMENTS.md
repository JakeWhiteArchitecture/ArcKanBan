...
72. **Task-list view** — a third project view (`/projects/<uid>/list`) alongside the board and decision register: the in-scope tasks grouped into **status segments** stacked vertically (done → awaiting → in progress → to do → upcoming → goals), each a native `<details>` you can collapse, each task tagged with its stage (+ sub-stage). An **✉ email** (`/list.eml`) sends the same list, segments collapsible via `<details>`/`<summary>` (works in browsers + clients that support it; degrades to expanded). New `build_task_list()`; linked from the board, decision register and home Config. No new JS.
