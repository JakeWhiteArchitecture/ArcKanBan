...
**Status:** v0.13 specification. A v0.1 prototype (Flask + SQLite) is referenced as the foundation but is **not present in this repository** — see *Repository state* below. This document specifies the build from that foundation forward.

> **Revision note (v0.13):** board & register polish. The card menu's **"Move to section"** is now a single item with a **flyout submenu** (opens right, flips left at the edge) so the menu stays short. The board **header** is grid-aligned — the **logo sits over column 1** and the **job no./name over column 2** — and **search** returns to the **right** group with a larger magnifier glyph. **Projects can be archived**: each project's ⚙ Config gains **Archive / Unarchive** (`projects.archived`); archived projects drop into a muted **"Archived"** section below the live grid (`POST /projects/<uid>/archive` · `/unarchive`). Home tagline reworded. Full deltas in §11.
...
### v0.10 → v0.11 (sub-stages in Config; register & reports sub-stage aware)
...
### v0.12 → v0.13 (board/header polish; archive projects)
60. **"Move to section" flyout submenu** — the card menu lists sections in a submenu that opens to the right of a single "Move to section" item (flips left near the screen edge; `ctxSubmenu` builds it as a child of the card menu so `closeMenu()` clears it). Keeps the menu short and fixes the over-tall menu on stages with many sections.
61. **Grid-aligned header** — the titleblock's left padding matches the slide's (logo lines up with column 1), and the logo occupies a column-1-wide slot (`max(180px, calc((100vw - 130px)/6 + 10px))`) so the **job no./name line up with column 2**, tracking the fluid 6-column grid down to the 860px breakpoint.
...
72. **Task-list view** — a third project view (`/projects/<uid>/list`) alongside the board and decision register: the **current stage's** tasks grouped into **status segments** stacked vertically (done → awaiting → in progress → to do → upcoming → goals), each a native collapsible `<details>`. The stage shows once (header / email subject), so each row carries its **section of work** instead (sub-stage-prefixed when the stage is split); **done tasks are greyed and struck through**. An **✉ email** (`/list.eml`) sends the same list — borderless tables, rounded segment headers, each segment collapsible via `<details>`/`<summary>` (degrades to expanded where unsupported). New `build_task_list()`; linked from the board, decision register and home Config. No new JS; the page scrolls (`page-tasklist` joins the register pages in `overflow:auto`).
73. **Home support chips + nav order** — the projects page gains two fixed bottom-right chips (home only): *Buy me a coffee* and the *GitHub* source repo, styled as glass pills. The decision register's title bar now lists **List** before **Board**.
74. **Interactive task-list view** (`static/tasklist.js`, new) — the list view becomes interactive, reusing the board's APIs (no new endpoints): **drag** to reorder within a status or move across statuses (`/api/tasks/<id>/move`, lane = stage+substage+status+section); a **hover tick-box** marks a task done / reopens it (`/api/tasks/<id>` `status`); a **RIBA stage pager** + **Show all stages** (mirrors the decision register; the per-task stage chip shows only in the all-stages view); the board's **Filter** (urgent / statutory / hide-done, same body classes); and a **right-click menu on decisions** to set the decision-maker and confirm an option / clear it (`/confirm`, `/unconfirm`, `awaiting_on`). All segments render (empty hidden) so drag/tick always have a target; the DONE segment also gets top breathing room under the header.

### v0.14 → v0.15 (Gantt chart)
75. **Gantt chart view** (`/projects/<uid>/gantt`, `static/gantt.js` + `templates/gantt.html`, new) — a fourth project view driven entirely by the existing **Sections of Work** (no new dependency concept, no bar dependencies). One row per section, listed in the sidebar across **every stage**; a bar on the row for any section scheduled onto the Gantt (`gantt_items`: start/end date + colour). The on-screen chart is an inline SVG built client-side: month grid, week ticks, a red **Today** line, alternating row stripes, and **holiday** blocks (`gantt_holidays`) drawn as a crosshatched overlay across the full chart height — any bar crossing a holiday is split either side of it (`_split_around_holidays`, shared logic re-implemented in both Python and JS). Bars are drawn from **seven corner-shape options** (rounded / chamfered / square, in various corner combinations) set project-wide via a **Settings cog** popup with a live preview (`projects.gantt_shape`, `POST /api/projects/<id>/gantt/settings`); the same corner map drives the reportlab PDF export so the two match. Clicking a bar (or a sidebar row's +/✎ button) opens an editor: start date, and end date/duration kept in sync (either recalculates the other); a **+ Holiday** button and clicking a holiday's label open the holiday editor. Seven small JSON endpoints (items × 3, holidays × 3, settings × 1) follow the existing validation/response conventions. **`GET /gantt.pdf`** renders an A3-landscape programme (title, month grid, right-aligned section titles, holiday crosshatch, the project's bar shape) via `reportlab` (new dependency); **`GET /gantt.eml`** sends the same PDF as an email attachment with a one-line covering note (extends the `.eml` pattern with `EmailMessage.add_attachment`). Linked from the board, decision register and task list headers.
