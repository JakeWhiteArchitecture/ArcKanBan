...
> **Revision note (v0.13):** board & register polish. The card menu's **"Move to section"** is now a single item with a **flyout submenu** (opens right, flips left at the edge) so the menu stays short. The board **header** is grid-aligned — the **logo sits over column 1** and the **job no./name over column 2** — and the **＋ New task** button moves to the **left** while **search** returns to the **right** (with a larger magnifier glyph). **Projects can be archived**: each project's ⚙ Config gains **Archive / Unarchive** (`projects.archived`); archived projects drop into a muted **"Archived"** section below the live grid (`POST /projects/<uid>/archive` · `/unarchive`). Home tagline reworded. Full deltas in §11.
...
62. **Header buttons swapped** — **＋ Task** moves to the left group; **search** returns to the right group with an enlarged magnifier glyph (clearer at a glance).
